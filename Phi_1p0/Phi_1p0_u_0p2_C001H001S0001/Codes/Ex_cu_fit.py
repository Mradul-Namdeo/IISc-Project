...
ORIGINAL_RAW_FILE = r"D:\FREI_videos_Flame_tracking\Phi_1p0\Phi_1p0_u_0p2_C001H001S0001\Datasets2\New_Phi_1p0_u_0p2_C001H001S0001.csv"
# The *new* 5-second CSV file that will be CREATED and then USED
SCALED_DATA_FILE = r"D:\FREI_videos_Flame_tracking\Phi_1p0\Phi_1p0_u_0p2_C001H001S0001\Datasets2\Phi_1p0_u_0p2_C001H001S0001_5sec.csv"
# The folder where all PLOTS and VIDEOS will be saved
PLOT_OUTPUT_DIR = r"D:\FREI_videos_Flame_tracking\Phi_1p0\Phi_1p0_u_0p2_C001H001S0001\Plots2"
# The folder where all DATA (parameter CSVs) will be saved
DATA_OUTPUT_DIR = r"D:\FREI_videos_Flame_tracking\Phi_1p0\Phi_1p0_u_0p2_C001H001S0001\Datasets2"
...
if transformation_success:
    ...
    try:
        ...
        for flame_id in flame_ids:
            ...
            C_fit = y_data.min()
            A_fit = y_data.max() - C_fit
            
            if A_fit < 1e-6: # Handle flat data
                print(f"Skipping Flame {flame_id}: Data is flat.")
                all_flame_data[flame_id] = {'x': x_data, 'y': y_data, 'params': None}
                continue

            def exp_model_2_param(t, k, t0):
                return saturating_exponential(t, A_fit, k, t0, C_fit)

            mid_rise_y = (A_fit / 2.0) + C_fit
            mid_rise_idx = np.argmin(np.abs(y_data - mid_rise_y))
            guess_t0 = x_data[mid_rise_idx]
            
            total_time = x_data.max() - x_data.min()
            guess_k = 4.0 / total_time if total_time > 0 else 1.0 # Heuristic
            
            initial_guesses_2_param = [guess_k, guess_t0]
            bounds_2_param = ([1e-3, x_data.min() - total_time], # [k_min, t0_min]
                              [500, x_data.max() + total_time])  # [k_max, t0_max]
            
            try:
                popt_2_param, _ = curve_fit(exp_model_2_param, x_data, y_data, 
                                            p0=initial_guesses_2_param, 
                                            bounds=bounds_2_param, 
                                            maxfev=10000)
                
                k_fit, t0_fit = popt_2_param
                ...
                print(f"Could not find a 2-param fit for Flame {flame_id}: {e}")
                all_flame_data[flame_id] = {'x': x_data, 'y': y_data, 'params': None}
