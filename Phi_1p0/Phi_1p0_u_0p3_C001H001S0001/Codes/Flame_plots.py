...
# --- Input/Output Paths ---
# Use os.path.join for better compatibility
DATASETS_DIR = r"D:\FREI_videos_Flame_tracking\Phi_1p0\Phi_1p0_u_0p3_C001H001S0001\Datasets2"
PLOTS_DIR = r"D:\FREI_videos_Flame_tracking\Phi_1p0\Phi_1p0_u_0p3_C001H0001\Plots2"

# The *original* unscaled CSV file (e.g., 83-second)
ORIGINAL_RAW_FILE = os.path.join(DATASETS_DIR, "Phi_1p0_u_0p3_C001H001S0001.csv")
# The *new* 5-second CSV file that will be CREATED and then USED
SCALED_DATA_FILE = os.path.join(DATASETS_DIR, "Phi_1p0_u_0p3_C001H001S0001_5sec.csv")
# The folder where all PLOTS and VIDEOS will be saved
PLOT_OUTPUT_DIR = PLOTS_DIR
# The folder where all DATA (parameter CSVs) will be saved
DATA_OUTPUT_DIR = DATASETS_DIR

# --- Analysis Parameters ---
# The target duration for the new scaled file
TARGET_DURATION_SEC = 5.0
# Which flames to process (slice(None) means all)
FLAME_SLICE = slice(None)

# --- Star Normalization Constants ---
# These are your characteristic values
T_STAR = 0.000261051  # Characteristic Time
P_STAR = 0.107         # Characteristic Position
V_STAR = 409.881       # Characteristic Velocity
...
def saturating_exponential(t, A, k, t0, C):
    ...
    safe_exp_arg = -k * (t - t0)
    safe_exp_arg = np.clip(safe_exp_arg, -700, 700) # Prevents overflow
...
def normalize_0_to_1(data):
    ...
    if range_val > 1e-9:
        ...
        return np.full_like(data, 0.5) # Return 0.5 for flat data
...
def create_star_norm_formula_image(params_list, filename):
    ...
    for params in params_list:
        ...
        # Apply the transformation rules using global constants
        A_star = A / P_STAR
        k_star = k * 100.0  # Note: This k_star is an arbitrary scaling, not from t_star
        t0_star = t0 / T_STAR
        C_star = C / P_STAR
        
        formula = f'$P_{{{flame_id}}}^{{\\star}}(t_{{\\star}}) = {A_star:.2f} \\left(1 - e^{{-{k_star:.2f}(t_{{\\star}} - {t0_star:.4f})}}\\right) + {C_star:.2f}$'
...
# --- 4. PART 1: DATA TRANSFORMATION (PRE-PROCESSING) ---
...
# --- 5. PART 2: CURVE FITTING AND ANALYSIS ---
# This part only runs if the transformation was successful
if transformation_success:
    ...
    try:
        ...
        # --- PART A: Perform Curve Fitting for each flame (FIXED-START & FIXED-ASYMPTOTE 1-PARAM FIT) ---
        print("\n--- Part A: Fitting Individual Flames (Fixed Start/Asymptote 1-Param Fit) ---")
        for flame_id in flame_ids:
            ...
            # --- MODIFIED: Define the fixed start point and asymptote ---
            t_start = x_data[0]
            P_start = y_data[0]
            P_max = y_data.max() # The asymptote is the max value of the flat data

            if (P_max - P_start) < 1e-6: # Handle flat data
                print(f"Skipping Flame {flame_id}: Data is flat.")
                all_flame_data[flame_id] = {'x': x_data, 'y': y_data, 'params': None}
                continue

            # --- MODIFIED: Pre-calculate A, t0, and C ---
            # These are now constants, not fitted parameters
            A_fit = P_max - P_start
            t0_fit = t_start
            C_fit = P_start

            # --- MODIFIED: Define a new 1-parameter model ---
            # We *only* fit for 'k' (steepness)
            def exp_model_1_param(t, k):
                # A, t0, and C are fixed from the pre-calculated values
                return saturating_exponential(t, A_fit, k, t0_fit, C_fit)

            # --- MODIFIED: Guesses and bounds for 'k' ONLY ---
            total_time = x_data.max() - x_data.min()
            guess_k = 4.0 / total_time if total_time > 0 else 1.0
            
            initial_guesses = [guess_k]
            bounds = ([1e-3], [500]) # [k_min], [k_max]
            
            safe_p0 = [np.clip(initial_guesses[0], bounds[0][0], bounds[1][0])]

            try:
                # --- MODIFIED: Call curve_fit to find only 'k' ---
                popt_fit, _ = curve_fit(exp_model_1_param, 
                                        x_data, y_data, 
                                        p0=safe_p0, 
                                        bounds=bounds, 
                                        maxfev=10000,
                                        loss='soft_l1') # Robust loss for in-between points
                
                k_fit = popt_fit[0]
                
                # --- MODIFIED: Reconstruct the full parameter list ---
                ...
                print(f"Could not find a 1-param fit for Flame {flame_id}: {e}")
                all_flame_data[flame_id] = {'x': x_data, 'y': y_data, 'params': None}
        ...
        for i, flame_id in enumerate(flame_ids):
            ...
            # For pinned plots, we MUST use the true start/end for smooth line
        ...
        plot_configs = {
            ...
            'norm_pos': ('plot_normalized_position.png', 'Normalized Position (X and Y)', 'Normalized Time (0 to 1)', 'Normalized Position (0 to 1)', True),
            'norm_vel': ('plot_normalized_velocity.png', 'Normalized Velocity (X and Y)', 'Normalized Time (0 to 1)', 'Normalized Velocity (0 to 1)', True),
            'norm_accel': ('plot_normalized_acceleration.png', 'Normalized Acceleration (X and Y)', 'Normalized Time (0 to 1)', 'Normalized Acceleration (0 to 1)', True),
            
            'star_norm_pos': ('plot_star_normalized_position.png', f'Star Normalized Position (t/{T_STAR}, P/{P_STAR})', f'Scaled Time (t/{T_STAR})', f'Star Normalized Position (P / {P_STAR})', True),
            'star_norm_vel': ('plot_star_normalized_velocity.png', f'Star Normalized Velocity (V / {V_STAR})', f'Scaled Time (t/{T_STAR})', f'Star Normalized Velocity (V / {V_STAR})', True),
            'star_norm_accel': ('plot_star_normalized_acceleration.png', 'Star Normalized Acceleration (Scaled Time, Y-Normalized)', f'Scaled Time (t/{T_STAR})', 'Normalized Acceleration (0 to 1)', True),
        }
